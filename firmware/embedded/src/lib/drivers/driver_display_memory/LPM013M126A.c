...
/* Dirty tracking: min/max lines touched since last refresh */
static int16_t dirty_y0 = 0;
static int16_t dirty_y1 = -1; /* -1 => no dirty */

/* EXTCOMIN periodic toggle to keep panel stable */
static void extcomin_toggle(void);
static void extcomin_timer_handler(struct k_timer *timer)
{
    ARG_UNUSED(timer);
    extcomin_toggle();
}
K_TIMER_DEFINE(extcomin_timer, extcomin_timer_handler, NULL);
...
static inline void dirty_mark_line(int16_t y)
{
    if (y < window_y || y >= window_y + window_h) return;
    if (dirty_y1 < dirty_y0) {
        dirty_y0 = y;
        dirty_y1 = y;
    } else {
...
int cmlcd_init(void)
{
    ...
    /* Start EXTCOMIN toggle at 1 Hz to maintain VCOM */
    k_timer_start(&extcomin_timer, K_SECONDS(1), K_SECONDS(1));
    return 0;
}
...
{
    if (dirty_y1 < dirty_y0) {
        /* Nothing changed, but still keep VCOM via timer */
        return;
    }
    ...
    int y_start = dirty_y0;
    int y_end   = dirty_y1;
    if (y_start < window_y) y_start = window_y;
    if (y_end   >= window_y + window_h) y_end = window_y + window_h - 1;
    ...
    {
        if (y >= LCD_DISP_HEIGHT)
            break;

        const int line_idx = y - window_y;

        memcpy(cmd_buf, bg_row, sizeof(cmd_buf));
        memcpy(&cmd_buf[window_x / 2], &disp_buf[(window_w / 2) * line_idx], copy_width);
        ...
        err = spi_write_bytes(head, sizeof(head), false, &lcd_cfg);          // MSB
        if (!err) err = spi_write_bytes(cmd_buf, LCD_DISP_WIDTH / 2, false, &lcd_cfg); // MSB
        if (!err) err = spi_write_bytes(tail, sizeof(tail), true, &lcd_cfg);           // LSB

        k_busy_wait(6);
        cs_set_active(false);

        if (err) {
            LOG_ERR("Refresh SPI failed at line %d (%d)", y, err);
            break;
        }
    }
    if (!err) {
        /* Reset dirty range after successful refresh */
        dirty_y0 = 0;
        dirty_y1 = -1;
        extcomin_toggle();
    }
